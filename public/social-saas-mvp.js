...
function syncFileInput(id,key){var input=q("#"+id),file=input&&input.files&&input.files[0];if(!file)return;if(fileState[key]&&fileState[key].url)URL.revokeObjectURL(fileState[key].url);fileState[key]={name:file.name,type:file.type,url:URL.createObjectURL(file)}}function clearFileInput(id,key){var input=q("#"+id);if(fileState[key]&&fileState[key].url)URL.revokeObjectURL(fileState[key].url);fileState[key]=null;if(input)input.value="";renderComposer()}function fileEntry(key){return fileState[key]}function setFileRemoveButton(button,entry){if(!button)return;var show=!!entry,item=button.closest&&button.closest(".upload-item");if(item)item.classList.toggle("has-file",show);button.classList.toggle("is-hidden",!show);button.hidden=!show;button.removeAttribute("title");button.setAttribute("aria-label",show?"Remove selected file":"");button.tabIndex=show?0:-1;if(!show){button.textContent="";button.style.setProperty("display","none","important");return}button.textContent="";button.style.setProperty("display","inline-flex","important");button.style.setProperty("align-items","center","important");button.style.setProperty("justify-content","center","important");button.style.setProperty("position","absolute","important");button.style.setProperty("top","10px","important");button.style.setProperty("right","10px","important");button.style.setProperty("width","30px","important");button.style.setProperty("height","30px","important");button.style.setProperty("min-width","30px","important");button.style.setProperty("min-height","30px","important");button.style.setProperty("padding","0","important");button.style.setProperty("border-radius","999px","important");button.style.setProperty("border","1px solid rgba(248,113,113,.72)","important");button.style.setProperty("background","rgba(239,68,68,.12)","important");button.style.setProperty("color","#fb7185","important");button.style.setProperty("font-size","19px","important");button.style.setProperty("font-weight","700","important");button.style.setProperty("line-height","1","important");button.style.setProperty("z-index","4","important")}function previewFileHtml(key,label,emptyText){var entry=fileEntry(key);if(!entry||!entry.url)return '<div class="preview-slot"><span>'+escapeHtml(emptyText)+'</span></div>';var isVideo=entry.type.indexOf("video/")===0,isImage=entry.type.indexOf("image/")===0;if(isVideo)return '<div class="preview-slot"><video src="'+escapeAttr(entry.url)+'" muted controls></video><span class="preview-label">'+escapeHtml(label)+'</span></div>';if(isImage)return '<div class="preview-slot '+(label==="封面"?"cover":"")+'"><img src="'+escapeAttr(entry.url)+'" alt="'+escapeAttr(label)+'"><span class="preview-label">'+escapeHtml(label)+'</span></div>';return '<div class="preview-slot"><span>'+escapeHtml(entry.name)+'</span></div>'}
